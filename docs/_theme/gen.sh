...
#   cargo build --release
#   PATH="$PWD/target/release:$PATH" sh docs/_theme/gen.sh
#
# Produces:
...
#   docs/hero.png  — static hero (block verdict), shader applied.
#
...
command -v npmguard >/dev/null 2>&1 \
  || { echo "npmguard binary not on PATH — run \`cargo build --release\` then prepend ./target/release to PATH" >&2; exit 1; }
